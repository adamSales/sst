...
################
#### minimal modifications by Adam; noted
###############
...
#source('src/cft/rdlocrand_fun.R')  ### ADAM CHANGED
...
rdwinselect = function(R, X,
    ...
    plot = FALSE,
    quietly = FALSE,
    edge=FALSE) {   ## ADAM ADDED THIS LAST OPTION
...
### ADAM CHANGED FOLLOWING

#  for (w in window.list){
  for(wNum in 1:length(window.list)){
      w <- window.list[wNum]  # NEW

      ## ADAM ADDED THIS if() else STATEMENT
      if(wNum>1 & edge){
          wPrev <- window.list[wNum-1]
          ww <- ((Rc >= -w) & (Rc < -wPrev)) | ((Rc <= w) & (Rc > wPrev))
      } else ww = (Rc >= -w) & (Rc <= w)
    ...
    if (!missing(X)){
      ...
      if (statistic=='hotelling'){
         ##  aux = try(hotelT2(Xw,Dw))
        ##   if(class(aux)=='try-error'){
        ##       window.list <- window.list[-wNum]
        ##       next
        ##   }
  ...
  output = list(window = rec.window,
      results = table.rdw,
      summary = table.sumstats)  ### ADAM CHANGED, THEN CHANGED BACK
