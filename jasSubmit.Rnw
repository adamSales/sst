...
Typically \citep[e.g.][]{imbensLemiuxRDD,lee}, regression
is used to model regression discontinuity designs, but
\citet{cft}, models $Y$ in a small bandwidth around $c$ as
independent of $R$ and other covariates, as if generated by a
...
Then, choose the largest bandwidth for which the independence assumption
...
Unlike under current approaches, an individual test result will itself not
...
\subsection{Lag Order in Autoregression Models: US Total Unemployment}
...
a ``running variable'' $R$, students' GPAs.
...
Bandwidth selection also plays an important role in more conventional
regression discontinuity analysis, which relies on regression modeling
of $Y$ as a function of $R$ and $Z$ \citep[e.g.][]{lee}.
Researchers will commonly
fit the regression models to a subset of the data with $R$ in a
window around $c$, to ensure robustness to model misspecification.
...
For the sake of simplicity, the example here will be limited to the
methods in \citet{cft}; however, the method applies to other styles of
...
ds <- pvalsRD$bws[unique(windowsT)]
dnames <- vapply(unique(na.omit(windowsT)),
                 function(ww)
...
ggplot(mapping=aes(x=pvalsRD$bws,y=pvalsRD$ps))+geom_point(size=2)+
    ...
    geom_label(mapping=aes(x=na.omit(ds),y=1.1,label=dnames),inherit.aes=FALSE)+
...
tab <- rbind(tab,
             IK=data.frame(bw=round(ik$bw[1],3),
                           ATE=paste0(
                               round(-ik$est[1],2),
                               ' (',round(-ik$ci[1,2],2),',',
                               round(-ik$ci[1,1],2),')')
                           ))
names(tab) <- c('Bandwidth','Effect (95\\% CI)')


xtab <- xtable(tab,caption="Selected regression discontinuity bandwidths ('d' is the point in the sequence selected, and 'bw' is the bandwidth) using covariate balance tests for all the covariates in \\citet{lso} and only high school GPA, respectively, along with their assosicated estimates for the average treatment effect of academic probation (ATE), with standard errors.",label="tab:RDD")
align(xtab) <- "rcc"
...
On the one hand, covariates appear to be approximately balanced for a range of
bandwidths.
...
Because the outcome of interest is itself a GPA, prior measures of GPA
