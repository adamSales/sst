...
(X\midr)-\EE(X\mid-r)=2\beta(r-10)$, where $\beta=$\Sexpr{beta}.
...
Figure \ref{fig:linImb} shows the results of \Sexpr{nsim} replications
...
\begin{figure}
<<linearImbalance,fig.width=6,fig.height=3,cache=TRUE>>=
...
\begin{figure}
<<sinusoidalImbalance,fig.width=6,fig.height=3,cache=TRUE>>=
facet(st,TRUE)
@
...
\begin{figure}
<<rdd,fig.width=6,fig.height=3,results='hide',cache=TRUE>>=
...
\begin{figure}
<<tsResults,fig.width=6,fig.height=3>>=
