...
\begin{figure}
<<linearImbalance,fig.width=6,fig.height=4,cache=TRUE>>=
...
\begin{figure}
<<sinusoidalImbalance,fig.width=6,fig.height=4,cache=TRUE>>=
facet(st,TRUE)
@
...
addtorow <- list()
addtorow$pos <- list(-1)
addtorow$command <- c("& \\multicolumn{2}{c}{All Covariates}&\\multicolumn{2}{c}{HS GPA}\\\\\n")
print(xtab,sanitize.text.function=function(x) x,add.to.row=addtorow)
